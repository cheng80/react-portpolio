...
[`malgeum-reference.pen`](../malgeum-reference.pen)은 원본 구조를 빠르게 파악하기 위한 편집 가능한 Desktop 참고 캔버스다.
...
이 파일은 레이어와 페이지 흐름을 참고하기 위한 구조용 디자인이다. 원본을 픽셀 단위로 자동 복제한 결과물은 아니다.
...
`.pen` 파일은 JSON 파싱, 노드 ID 중복, 이미지 경로 존재 여부도 확인했다.
...
현재 변경사항은 아직 커밋되지 않았다. 기존 스타터 파일 삭제와 새 컴포넌트·이미지·Pen 파일 추가가 함께 존재하므로 다음 세션에서 변경사항을 되돌리거나 덮어쓰지 말고 먼저 `git status --short`를 확인해야 한다.

커밋, push, PR 작업은 아직 수행하지 않았다.
